...
func Run(ctx *cli.Context) error {
	...
	for !state.GetExited() {
		...
		if proofAt(state) {
			...
			if err := jsonutil.WriteJSON(proof, ioutil.ToStdOutOrFile(fmt.Sprintf(proofFmt, step), OutFilePerm)); err != nil {
	...
	if debugInfoFile := ctx.Path(RunDebugInfoFlag.Name); debugInfoFile != "" {
		if err := jsonutil.WriteJSON(vm.GetDebugInfo(), ioutil.ToStdOutOrFile(debugInfoFile, OutFilePerm)); err != nil {
