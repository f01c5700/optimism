...
func Init(cfg InitConfig) error {
	...
	if cfg.Dev {
		keygen, err := NewMnemonicKeyGenerator(devMnemonic)
		if err != nil {
	...
	if err := state.WriteTOMLFile(path.Join(cfg.Outdir, "intent.toml"), intent); err != nil {
		return fmt.Errorf("failed to write intent to file: %w", err)
	}
	if err := state.WriteJSONFile(path.Join(cfg.Outdir, "state.json"), st); err != nil {
